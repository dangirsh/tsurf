...
`extras/dev-agent.nix` is the core autonomous-agent module for unattended workflows. `extras/dashboard.nix` and `extras/cost-tracker.nix` are optional utilities. All extras are opt-in via explicit host imports.
