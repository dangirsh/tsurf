---
phase: 17-hardcore-simplicity-security-audit
plan: 03
type: execute
wave: 2
depends_on: ["17-01", "17-02"]
files_modified:
  - CLAUDE.md
autonomous: true

must_haves:
  truths:
    - "CLAUDE.md contains explicit security rules agents must follow"
    - "CLAUDE.md contains simplicity conventions to prevent future bloat"
    - "CLAUDE.md contains a module change checklist"
    - "CLAUDE.md project structure accurately reflects the current codebase"
    - "CLAUDE.md documents accepted risks (SEC5, SEC11) and deferred items (SEC3, SEC9)"
    - "nix flake check still passes (no code changes in this plan)"
  artifacts:
    - path: "CLAUDE.md"
      provides: "Security rules, simplicity conventions, module change checklist, accurate project structure"
      contains: "Security Conventions"
  key_links:
    - from: "CLAUDE.md"
      to: "modules/networking.nix"
      via: "internalOnlyPorts assertion reference"
      pattern: "internalOnlyPorts"
---

<objective>
Update CLAUDE.md with security guardrails, simplicity conventions, and accurate project documentation.

Purpose: Plans 01 and 02 fixed the existing issues. This plan establishes guardrails so agents (and humans) maintain these standards going forward. CLAUDE.md is the primary instruction file that all Claude sessions read -- conventions here are enforced by agent behavior.

Output: CLAUDE.md with Security Conventions, Simplicity Conventions, and Module Change Checklist sections. Project structure and key decisions updated to match reality.
</objective>

<execution_context>
@/home/ubuntu/.claude/get-shit-done/workflows/execute-plan.md
@/home/ubuntu/.claude/get-shit-done/templates/summary.md
</execution_context>

<context>
@.planning/PROJECT.md
@.planning/ROADMAP.md
@.planning/STATE.md
@.planning/phases/17-hardcore-simplicity-security-audit/17-RESEARCH.md

# Pre-flight: These summaries MUST exist before this plan executes (created by Plans 01 and 02).
# If either is missing, the executor should warn and proceed using the RESEARCH.md as the
# authoritative source for what was implemented, since Plans 01/02 follow RESEARCH.md directly.
@.planning/phases/17-hardcore-simplicity-security-audit/17-01-SUMMARY.md
@.planning/phases/17-hardcore-simplicity-security-audit/17-02-SUMMARY.md
</context>

<tasks>

<task type="auto">
  <name>Task 1: Update project structure and key decisions</name>
  <files>CLAUDE.md</files>
  <action>
**Pre-flight check:** Before starting, verify that 17-01-SUMMARY.md and 17-02-SUMMARY.md exist:
```bash
ls .planning/phases/17-hardcore-simplicity-security-audit/17-01-SUMMARY.md
ls .planning/phases/17-hardcore-simplicity-security-audit/17-02-SUMMARY.md
```
If either is missing, warn in the task output but continue -- use 17-RESEARCH.md as the authoritative source for what Plans 01/02 implemented, since those plans follow the research findings directly.

The current CLAUDE.md has an outdated project structure (references `common.nix`, `dev-tools.nix`, `docker-services.nix`, `agent-tooling.nix`, `shell.nix` which no longer exist, and is missing many current modules). Update it to reflect the actual codebase.

Replace the `## Project Structure` section with the accurate module layout:

```
flake.nix              # Entrypoint — inputs (7), outputs, nixosConfigurations.acfs
flake.lock             # Pinned dependencies (nixpkgs 25.11, home-manager, sops-nix, disko, parts, claw-swap, llm-agents)
hosts/
  acfs/
    default.nix        # Host-specific NixOS config (imports all modules)
    hardware.nix       # Hardware/disk config (disko, Contabo VPS)
modules/
  default.nix          # Import hub for all modules
  base.nix             # Nix settings, system packages, kernel sysctl hardening
  boot.nix             # GRUB bootloader config
  networking.nix       # Firewall (nftables), SSH (Tailscale-only), Tailscale, fail2ban
  users.nix            # User accounts, sudo, SSH authorized keys
  secrets.nix          # sops-nix secret declarations
  docker.nix           # Docker engine (--iptables=false), NAT
  monitoring.nix       # Prometheus + node_exporter + alert rules
  syncthing.nix        # Syncthing file sync service
  home-assistant.nix   # Home Assistant + ESPHome
  homepage.nix         # Homepage dashboard (Tailscale-only)
  agent-compute.nix    # Agent CLI (claude, codex), bubblewrap sandbox, Podman, agent-spawn
  repos.nix            # Idempotent repo cloning on activation
  restic.nix           # Restic backup to Backblaze B2
home/
  default.nix          # home-manager import hub
  bash.nix             # Bash shell + API key exports
  git.nix              # Git + gh CLI config
  ssh.nix              # SSH client config
  direnv.nix           # Direnv auto-loading
  cass.nix             # CASS indexer timer
  agent-config.nix     # ~/.claude and ~/.codex symlinks
packages/
  zmx.nix              # Pre-built zmx terminal multiplexer binary
  cass.nix             # Pre-built CASS indexer binary
scripts/
  deploy.sh            # Deploy script (local/remote build, locking, container health check)
secrets/
  acfs.yaml            # sops-encrypted secrets (7 secrets + 1 template)
```

Update the `## Key Decisions` section to reflect current state:

- **Flakes + home-manager**: Modern, reproducible, lockfile-pinned (nixos-25.11)
- **Docker stays**: Containers declared in Nix (parts, claw-swap), `--iptables=false`
- **Restic to B2**: Automated daily backups to Backblaze B2 (S3 API)
- **sops-nix secrets**: All credentials encrypted, decrypted at activation via age keys
- **Agent tooling**: llm-agents overlay provides claude-code + codex; bubblewrap sandbox via agent-spawn
- **SSH via Tailscale only**: Port 22 not on public firewall; deploy uses Tailscale MagicDNS
- **Kernel hardening**: sysctl settings restrict dmesg, kptr, BPF, ICMP redirects

Update the `## Conventions` section:

- One module per concern (networking, services, dev-tools)
- Secrets managed via sops-nix (age key derived from SSH host key)
- All service configs are declarative -- no imperative setup steps
- Infrastructure repos cloned via activation scripts (clone-only, never pull)
- Internal services use `openFirewall = false` + `trustedInterfaces` (Tailscale-only)
- `@decision` annotations on security-relevant choices in module headers
  </action>
  <verify>
Read CLAUDE.md and confirm:
- Project Structure matches `ls modules/ home/ packages/ scripts/`
- Key Decisions includes sops-nix, Tailscale-only SSH, kernel hardening
- Conventions includes openFirewall=false pattern and @decision annotations
  </verify>
  <done>
CLAUDE.md project structure accurately reflects the current codebase. Key decisions and conventions are up to date.
  </done>
</task>

<task type="auto">
  <name>Task 2: Add security and simplicity guardrails</name>
  <files>CLAUDE.md</files>
  <action>
Add three new sections to CLAUDE.md after the existing `## Conventions` section. These establish the guardrails that prevent regression of the Phase 17 fixes.

**Section 1: `## Security Conventions`**

```markdown
## Security Conventions

Rules that agents MUST follow when modifying any module:

- **Never** add ports to `networking.firewall.allowedTCPPorts` -- the port 22 assertion and `internalOnlyPorts` assertion in `networking.nix` enforce this at build time, but do not attempt to weaken or remove these assertions
- **Never** commit unencrypted secrets to any file -- use sops-nix for all credentials
- **Never** embed credentials in URLs or command-line arguments -- use environment variables, credential helpers, or file-based injection
- **Never** weaken the bubblewrap sandbox defaults in `agent-compute.nix` -- `--no-sandbox` is for trusted operations only
- **Never** run Claude Code or Codex with `--no-sandbox` unless explicitly instructed by the user for a specific trusted operation -- sandbox is the default for a reason
- **Never** mount the Docker socket into a service unless strictly required and documented with a @decision annotation
- All network-facing services MUST have `openFirewall = false` (Tailscale-only via `trustedInterfaces`)
- New services MUST add their port to `internalOnlyPorts` in `networking.nix` if they should not be public
- All new modules MUST include `@decision` annotations for security-relevant choices
- Pre-built binaries (packages/*.nix) use SHA256 hash verification -- accepted risk, no signature verification available (SEC11)

### Accepted Risks (documented, not actionable)

- **SEC5:** `--no-sandbox` mode allows agents to modify `~/.claude/settings.json` -- mitigated by requiring explicit opt-in
- **SEC11:** Pre-built binaries (zmx, cass) lack signature verification -- mitigated by SHA256 hash pinning
- **SEC3:** Docker container hardening (read-only rootfs, cap-drop, no-new-privileges) is deferred -- containers are declared in external repos (parts, claw-swap), changes needed there
- **SEC9:** Systemd service hardening (ProtectHome, PrivateTmp) is deferred -- NixOS service modules provide baseline defaults, custom overrides risk breaking services
```

**Section 2: `## Simplicity Conventions`**

```markdown
## Simplicity Conventions

Rules to prevent bloat and over-engineering:

- Every new module must justify its existence -- prefer adding to existing modules over creating new ones for <20 lines
- No dead code -- unused options, packages, or features must be removed immediately
- YAGNI -- do not add features "for later" unless they are in an active phase plan
- One source of truth -- packages declared in exactly one module (no duplicates across modules)
- Prefer inline over separate files for small configs (<20 lines)
- Let bindings for values used more than once (e.g., Tailscale IP in homepage.nix)
- `tmp/` in project root for temporary files (never `/tmp/`) -- convention from global CLAUDE.md
```

**Section 3: `## Module Change Checklist`**

```markdown
## Module Change Checklist

Before committing any module change, verify:

1. **Port exposure:** Does this change expose a new port? Add to `internalOnlyPorts` in `networking.nix`, or justify public exposure with a @decision annotation
2. **Secret handling:** Does this change add a new secret? Verify owner/permissions are minimal in `secrets.nix`
3. **New service:** Does this change add a service? Set `openFirewall = false` and add @decision annotation
4. **Sandbox impact:** Does this change modify `agent-compute.nix`? Verify `/run/secrets` and `~/.ssh` remain hidden from sandboxed agents
5. **Credentials:** Does this change handle tokens or API keys? Use env vars or sops-nix, never URLs or CLI args
6. **Validation:** `nix flake check` passes
```

Place these three sections after `## Conventions` and before any existing `## Testing` section.
  </action>
  <verify>
Read CLAUDE.md and confirm:
- "Security Conventions" section exists with "Never" rules
- "Simplicity Conventions" section exists with YAGNI rule
- "Module Change Checklist" section exists with 6 items
- "Accepted Risks" subsection documents SEC3, SEC5, SEC9, SEC11
  </verify>
  <done>
CLAUDE.md contains explicit security rules (never expose ports, never embed credentials, never weaken sandbox, never run --no-sandbox without user instruction), simplicity conventions (no dead code, YAGNI, one source of truth), a module change checklist, and documented accepted risks. Agents reading CLAUDE.md will follow these guardrails.
  </done>
</task>

</tasks>

<verification>
- CLAUDE.md has accurate project structure matching actual files
- CLAUDE.md has Security Conventions with "Never" rules
- CLAUDE.md has Simplicity Conventions with YAGNI
- CLAUDE.md has Module Change Checklist with 6 items
- CLAUDE.md documents accepted risks (SEC3, SEC5, SEC9, SEC11)
- No code files were modified (guardrails only)
- `nix flake check` still passes (unchanged from Plan 01/02)
</verification>

<success_criteria>
CLAUDE.md is the single source of truth for security and simplicity standards. Any agent working on this codebase will read these rules before making changes. The guardrails specifically prevent regression of the Phase 17 fixes (port 22, credential leaks, sandbox weakening, dead code).
</success_criteria>

<output>
After completion, create `.planning/phases/17-hardcore-simplicity-security-audit/17-03-SUMMARY.md`
</output>
