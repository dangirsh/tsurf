...
# scripts/deploy-detached.sh — detached remote activation helper for deploy.sh.
set -euo pipefail
...
usage() {
  cat <<USAGE
Usage: $(basename "$0") --node <NAME> --target <USER@HOST> [OPTIONS]

Options:
  --flake-dir DIR        Flake directory (default: repository root)
  --node NAME            Deploy node name (required)
  --target U@H           SSH target (default: root@<node>)
  --first-deploy         Accepted for deploy.sh compatibility
  --magic-rollback       Accepted for deploy.sh compatibility
  --no-magic-rollback    Accepted for deploy.sh compatibility
  --help                 Show this help
USAGE
}
...
wait_remote_deploy() {
  ...
  local unit_q log_q exit_q status_q payload exit_code status unit_state stopped_without_exit
...
DEPLOY_EXIT=0
run_remote_deploy || DEPLOY_EXIT=$?
if [[ "$DEPLOY_EXIT" -ne 0 ]]; then
  echo "=== Detached deploy FAILED (exit $DEPLOY_EXIT) ==="
  exit 1
fi
