...
source "$ROOT_DIR/extras/scripts/deploy.sh"
...
assert_eq \
  "$(resolve_flake_dir "$ROOT_DIR/extras/scripts/deploy.sh")" \
  "$ROOT_DIR" \
  "resolve_flake_dir finds repo root from extras/scripts path"

WORKDIR="${TSURF_TEST_TMPDIR:-$PWD}/deploy-script"
rm -rf "$WORKDIR"
mkdir -p "$WORKDIR/scripts"
cp "$ROOT_DIR/extras/scripts/deploy.sh" "$WORKDIR/scripts/deploy.sh"
