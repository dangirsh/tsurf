...
1. **Eval checks (offline, fast, ~30s)** -- `nix flake check`
   - Validates NixOS config evaluation for both `neurosys` and `ovh` hosts.
...
3. **Live tests (SSH to running hosts)** -- `nix run .#test-live -- --host neurosys`
...
- `neurosysCfg` -- services host (Contabo)
- `ovhCfg` -- dev host (OVH)
...
1. Read `nix flake check` output. The check name (for example `firewall-ports-neurosys`) maps directly to a derivation in `config-checks.nix`.
