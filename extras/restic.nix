...
in
{
  options.services.resticStarter = {
    enable = lib.mkEnableOption "Restic B2 backup";

    repository = lib.mkOption {
      type = lib.types.nullOr lib.types.str;
      default = null;
      example = "s3:s3.eu-central-003.backblazeb2.com/my-bucket";
      description = "Restic repository URL. Required when resticStarter is enabled.";
    };
  };
    ...
    assertions = [
      {
        ...
        message = "services.resticStarter.repository must be set when resticStarter is enabled.";
      }
    ];
