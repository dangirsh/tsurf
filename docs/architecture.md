...
- The wrapper refuses to run inside a repo marked with `.tsurf-control-plane`.
...
The `agent` user is in `wheel`, but sudo is limited to immutable launcher
binaries generated by `modules/agent-launcher.nix`. The agent user is kept out
of `docker`, and raw provider secrets remain root-owned by default.

## Network And Persistence Model

- Public ingress is limited to SSH on `22`, plus `80` and `443` only when
  `services.nginx.enable = true`.
- The public network model does not trust `tailscale0`. Tailnet exposure is a
  private-overlay concern.
...
## First-Class Public Agent Paths

- `claude` is the public interactive path in core.
- `extras/dev-agent.nix` provides the public unattended Claude service.
